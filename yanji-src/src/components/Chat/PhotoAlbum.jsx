...
import { albumImage, conversationCard, hideAlbumItem, listAlbum, prepareAlbumImage, restoreAlbumItem, saveAlbum } from '../../api/album'
...
function PrivateImage({ config, item, thumbnail, trash = false, ...props }) {
  const [url, setUrl] = useState('')
  const [failed, setFailed] = useState(false)
  useEffect(() => {
    let active = true; let objectUrl
    setUrl(''); setFailed(false)
    albumImage(config, item.id, thumbnail, trash).then(blob => {
      if (!active) return
      objectUrl = URL.createObjectURL(blob); setUrl(objectUrl)
    }).catch(() => { if (active) setFailed(true) })
    return () => { active = false; if (objectUrl) URL.revokeObjectURL(objectUrl) }
  }, [config.baseUrl, config.apiToken, item.id, thumbnail, trash])
...
export default function PhotoAlbum({ onClose, messages = [] }) {
  ...
  async function refresh() {
    ...
    try {
      const result = await listAlbum(config, '', trashMode)
  ...
  }, [config?.baseUrl, config?.apiToken, config?.enabled, trashMode])
  useEffect(() => {
    const close = event => {
  ...
  async function turn(direction) {
    ...
    try {
      const result = await listAlbum(config, cursor, trashMode)
  ...
  async function remove() {
    if (!await confirm({ title: '移入回收站？', description: '30 天内可以恢复；到期后会自动彻底删除并释放空间。', confirmLabel: '移入回收站' })) return
    setBusy(true); setError('')
  ...
  async function restore() {
    setBusy(true); setError('')
    try { await restoreAlbumItem(config, item.id); await refresh() }
    catch (e) { setError(e.message) }
    finally { setBusy(false) }
  }

  const trashDaysLeft = item?.deleted_at ? Math.max(0, 30 - Math.floor((Date.now() - new Date(item.deleted_at).getTime()) / 86400000)) : 30
  ...
  return createPortal(<div className="photo-album-overlay" onClick={() => { if (!busy) onClose() }}>
    <section className="photo-album" role="dialog" aria-modal="true" aria-label="共同相册" onClick={e => e.stopPropagation()}>
      <header className="photo-album-head"><div><h2><AlbumIcon />{trashMode ? '回收站' : '相册'}</h2><p>{trashMode ? '软删除的照片保留 30 天' : '把想留下的这一刻，翻给你看'}</p></div><div className="photo-album-head-actions"><button className="photo-album-trash-toggle" onClick={() => { setDetail(false); setTrashMode(value => !value) }} disabled={busy}>{trashMode ? '返回相册' : '回收站'}</button><button onClick={onClose} disabled={busy} aria-label="关闭相册">×</button></div></header>
      ...
      {draft ? <form className="photo-album-editor" onSubmit={keepDraft}>
        ...
        <div className="photo-album-full-image"><PrivateImage config={config} item={item} trash={trashMode} /></div>
        <h3>{item.title}</h3><p>{item.description || '这一刻，无需多说。'}</p>
        <small>{trashMode ? `回收站还会保留 ${trashDaysLeft} 天` : `${item.author} 收藏 · ${new Date(item.created_at).toLocaleDateString('zh-CN')}`}</small>
        {item.source_url && /^https:\/\//i.test(item.source_url) && <a href={item.source_url} target="_blank" rel="noopener noreferrer">查看原始来源 ↗</a>}
        {trashMode ? <button className="photo-album-restore" onClick={restore} disabled={busy}>恢复到相册</button> : <button className="photo-album-back" onClick={remove} disabled={busy}>移入回收站</button>}
      </div> : <>
        <div className="photo-album-book" onTouchStart={e => { swipe.current = { x: e.touches[0].clientX, y: e.touches[0].clientY } }} onTouchCancel={() => { swipe.current = null }} onTouchEnd={e => {
          ...
          {loading ? <div className="photo-album-empty">正在翻开相册…</div> : item ? <button key={item.id} className="photo-album-page" onClick={() => { if (Date.now() - lastSwipe.current > 350) setDetail(true) }} aria-label={`展开：${item.title}`}>
            <div className="photo-album-thumb"><PrivateImage config={config} item={item} thumbnail trash={trashMode} /></div><h3>{item.title}</h3><p>{item.description || '这一刻，无需多说。'}</p><small>{trashMode ? `剩 ${trashDaysLeft} 天 · 点照片恢复` : `${item.author} 收藏 · 点照片看详情`}</small>
          </button> : <div className="photo-album-empty"><AlbumIcon size={44} /><p>{error ? '还没能打开相册' : trashMode ? '回收站是空的。' : '第一页，等一张想留下的照片。'}</p></div>}
        </div>
        ...
        {!trashMode && <div className="photo-album-actions"><button onClick={() => fileRef.current?.click()} disabled={!connected || busy}>从手机选照片</button><button onClick={() => { setError(''); setPickChat(true) }} disabled={!connected || busy}>收藏聊天</button></div>}
      </>}
