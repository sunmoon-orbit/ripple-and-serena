...
class ChecklistWidget : AppWidgetProvider() {
    override fun onUpdate(context: Context, manager: AppWidgetManager, ids: IntArray) {
        ids.forEach { manager.updateAppWidget(it, loading(context)) }
        val pending = goAsync()
        CoroutineScope(Dispatchers.IO).launch {
            val rows = try { JSONArray(WidgetApi.request(context, "/checklist")) } catch (_: Exception) { null }
            ids.forEach { manager.updateAppWidget(it, render(context, rows)) }
            pending.finish()
        }
    }
    ...
    private fun loading(context: Context) = RemoteViews(context.packageName, R.layout.checklist_widget_layout).apply {
    ...
    private fun render(context: Context, rows: JSONArray?): RemoteViews {
