...
const tools = [
  ...
  { name: 'save_album_image', description: '把找到的公开 HTTPS 图片直链，或当前工具可读取的图片 base64，收藏到共同相册。保留真实来源和自己的描述，不要编造链接。图片不超过 4MB。', inputSchema: { type: 'object', properties: { title: { type: 'string' }, description: { type: 'string' }, author: { type: 'string' }, image_url: { type: 'string' }, image_data: { type: 'string', description: '与 image_url 二选一，data:image/...;base64,...。必须真实读取附件，不可猜造。' }, source_url: { type: 'string' } }, required: ['title', 'description', 'author'] } },
